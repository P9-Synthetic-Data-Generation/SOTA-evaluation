...
def pategan_main(args):
    ...
    for model_index in range(len(models)):
        ...
        # Using original data
        results[model_index, 0], results[model_index, 2] = supervised_model_training(
            train_data[:, : (data_dim - 1)],
            np.round(train_data[:, (data_dim - 1)]),
        ...
        results[model_index, 1], results[model_index, 3] = supervised_model_training(
            synth_train_data[:, : (data_dim - 1)],
    ...
    results = pd.DataFrame(
        np.round(results, 4), columns=["AUC-Original", "AUC-Synthetic", "APR-Original", "APR-Synthetic"]
    )
    print(results)
    print("Averages:")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dim", help="number of dimensions of generated dimension (if random)", default=10, type=int
    )
    ...
    parser.add_argument(
        "--iterations", help="number of iterations for handling initialization randomness", default=50, type=int
    )
