...
class AmortizedAccountant(object):
    ...
    def accumulate_privacy_spending(self, eps_delta, unused_sigma, num_examples):
        ...
        with tf.control_dependencies([tf.Assert(tf.greater(delta, 0), ["delta needs to be greater than 0"])]):
            ...
            return tf.group(
                *[
                    tf.assign_add(self._eps_squared_sum, tf.square(amortize_eps)),
...
class MomentsAccountant(object):
    ...
    def __init__(self, total_examples, moment_orders=32):
        ...
        self._log_moments = [
            tf.Variable(numpy.float64(0.0), trainable=False, name=("log_moments-%d" % moment_order))
...
class GaussianMomentsAccountant(MomentsAccountant):
    ...
    def _differential_moments(self, sigma, s, t):
        ...
        exponents = tf.constant(
            [j * (j + 1.0 - 2.0 * s) / (2.0 * sigma * sigma) for j in range(t + 1)], dtype=tf.float64
        )
    ...
    def _compute_log_moment(self, sigma, q, moment_order):
        ...
        qs = tf.exp(
            tf.constant([i * 1.0 for i in range(moment_order + 1)], dtype=tf.float64)
            * tf.cast(tf.log(q), dtype=tf.float64)
        )
...
def BatchClipByL2norm(t, upper_bound, name=None):
    ...
    with tf.name_scope(values=[t, upper_bound], name=name, default_name="batch_clip_by_l2norm") as name:
...
def SoftThreshold(t, threshold_ratio, name=None):
    ...
    with tf.name_scope(values=[t, threshold_ratio], name=name, default_name="soft_thresholding") as name:
        saved_shape = tf.shape(t)
        t2 = tf.reshape(t, tf.concat(axis=0, values=[tf.slice(saved_shape, [0], [1]), -1]))
        t_abs = tf.abs(t2)
        t_x = tf.sign(t2) * tf.nn.relu(t_abs - (tf.reduce_mean(t_abs, [0], keep_dims=True) * threshold_ratio))
